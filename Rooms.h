...
public:
	...
	Room* CreateNewRoom(std::string id, uWS::WebSocket<uWS::SERVER> *ws, message_handler mh, connection_handler ch,  disconnection_handler dh);
	bool JoinRoom(std::string roomID, uWS::WebSocket<uWS::SERVER> *user);
private:
	void AttachBasicHandlers(Room* room, message_handler mh, connection_handler ch, disconnection_handler dh);
private:
	rooms_map rooms;
	uWS::Hub *hub;
};